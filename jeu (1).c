...
// MATRICE POIDS ROI
int roiN_poids[8][8] = { {20, 30, 10,  5,  5, 10, 30, 20},
                         {20, 20,  5,  5,  5,  5, 20, 20},
                        {-10,-20,-20,-20,-20,-20,-20,-10},
                        {-20,-30,-30,-40,-40,-30,-30,-20},
                        {-30,-40,-40,-50,-50,-40,-40,-30},
...
int estim( struct config conf )
{
	...
	for (i=0; i<8; i++)
	   for (j=0; j<8; j++) {
		...
		switch (conf.mat[i][j]) {
		    //BLANC
		   case 'p' : pionB++; poids_totalB+=pionB_poids[i][j]; break;
		   case 'c' : cB++; poids_totalB+=chevalierB_poids[i][j]; break;
		   case 'f' : fB++; poids_totalB+=fouB_poids[i][j];    break;
		   case 't' : tB++; poids_totalB+=tourB_poids[i][j];  break;
		   case 'n' : nB++; poids_totalB+=reineB_poids[i][j];  break;
           case 'r' : if(ending_game(conf)==0) poids_totalB+=roiB_poids[i][j];
                      else poids_totalB+=fin_roiB_poids[i][j]; break;

            //NOIR
		   case -'p' : pionN++; poids_totalN+=pionN_poids[i][j]; break;
		   case -'c' : cN++; poids_totalN+=chevalierN_poids[i][j];  break;
		   case -'f' : fN++; poids_totalN+=fouN_poids[i][j]; break;
		   case -'t' : tN++; poids_totalN+=tourN_poids[i][j]; break;
		   case -'n' : nN++; poids_totalN+=reineN_poids[i][j]; break;
           case -'r' : if(ending_game(conf)==0) poids_totalN+=roiN_poids[i][j];
	...
	//materials
	nombrePiece = ((pionB*1 + cB*3 +fB*3 + tB*5 + nB*9+ 20*rB) - (pionN*1 + cN*3 + fN*3 + tN*5 + nN*9+ 20*rN))*1.2 ;

    //Weight_pawns
    poids = (poids_totalB - poids_totalN) * (0.5);



   //ScrQte = (poids );
  ScrQte = (nombrePiece + poids )/2;
...
void init( struct config *conf )
{
    ...
    conf->mat[0][2] = 'f'; conf->mat[0][3] =  'n';
	conf->mat[0][4] =  'r'; conf->mat[0][5] =  't';
